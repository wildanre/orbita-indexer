import { interestAccrued } from "ponder:schema";
import { datetimeFormatter } from "../../helper/datetimeFormatter";
import { dateFormatter } from "../../helper/dateFormatter";
import { timeFormatter } from "../../helper/timeFormatter";

export const handleInterestAccrued = async (event: any, context: any) => {
  await context.db.insert(interestAccrued).values({
    id: event.id,
    routerAddress: event.log.address,
    interest: event.args.interest,
    supplyYield: event.args.supplyYield,
    reserveYield: event.args.reserveYield,
    contractChainId: context.chain.id,
    txHash: event.transaction.hash,
    timestamp: event.block.timestamp,
    datetime: datetimeFormatter(event.block.timestamp),
    date: dateFormatter(event.block.timestamp),
    time: timeFormatter(event.block.timestamp),
  });
};
