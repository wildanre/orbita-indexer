...
export const handleWithdrawCollateral = async (event: any, context: any) => {
  ...
  // Update lending pool rate with collateral token for JSON tracking (withdraw = reduce)
  await handleLendingPoolRate(
    ...
    collateralToken,
    BigInt(0), // liquidity
    _amount, // collateral (to be decreased)
    BigInt(0), // borrow
    context,
    {
      liquidity: "+",
  ...
  // Update position detail - decrease collateral amount
